...
class Trainer:
    ...
    def train(self, save_dir, batch_gen, num_epochs, batch_size, learning_rate, device):
        ...
        optimizer = optim.Adam(self.model.parameters(), lr=learning_rate, weight_decay=5e-4)
        for epoch in range(num_epochs):
            self.adjust_learning_rate(optimizer, epoch, [35,60], learning_rate )
            epoch_loss = 0
            correct = 0
            total = 0
    ...
    def adjust_learning_rate(self, optimizer, epoch, lr_steps, learning_rate):
        """Sets the learning rate to the initial LR decayed by 10 every 30 epochs"""
        decay = 0.25 ** (sum(epoch >= np.array(lr_steps)))
